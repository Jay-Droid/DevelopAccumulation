...
public class IdCardCheckUtil {
  ...
  public static String IDCardValidate(String IDStr) {
    try {
      ...
      if (IDStr.contains("x")) IDStr.replaceAll("x", "X");
      if (IDStr.length() == 18) {
        if (!Ai.equals(IDStr)) {
  ...
  public static boolean isMale(String id) {
    ...
    if (!result.isEmpty()) {
      if (id.length() == 15) {
        return Integer.parseInt(id.substring(14, 15)) % 2 == 0 ? false : true;
      } else if (id.length() == 18) {
        return Integer.parseInt(id.substring(16, 17)) % 2 == 0 ? false : true;
      }
    }
    return true;
  }
