...
public class JavaListActivity extends AppCompatActivity {
  ...
  private List<DemoListAdapter.DemoItem> getDemoData() {
    ...
    demoList.add(new DemoListAdapter.DemoItem("泛型", "Java 泛型详解", ReflectionActivity.class));

    demoList.add(new DemoListAdapter.DemoItem("反射", "反射在Android中的运用", ReflectionActivity.class));

    demoList.add(new DemoListAdapter.DemoItem("动态代理", "动态代理实现页面路由框架", DynamicProxyActivity.class));
    demoList.add(new DemoListAdapter.DemoItem("多线程", "Android 线程信息", ThreadActivity.class));
    return demoList;
  }
